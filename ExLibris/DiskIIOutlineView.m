...
#import "DiskIIOutlineView.h"

@implementation DiskIIOutlineView
...
- (NSMenu *)menuForEvent:(NSEvent *)event {
  ...
  // Preserves the selection if the row under the mouse is selected (to allow
  // for
  ...
  if (self.numberOfSelectedRows <= 0) {
    // No rows are selected, so the table should be displayed with all items
    // disabled
