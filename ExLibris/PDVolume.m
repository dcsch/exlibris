...
static void bitPos(NSUInteger blockNumber, NSUInteger *byteIndex,
                   NSUInteger *bitIndex, NSUInteger *blockIndex) {
...
+ (void)formatBlockStorage:(BlockStorage *)blockStorage {
  ...
  PDDirectoryHeader *dirHead =
      [[PDDirectoryHeader alloc] initWithVolume:nil
                                parentDirectory:nil
                                    parentEntry:nil
                                          bytes:bytes + 4
                                         length:0];
...
- (instancetype)initWithContainer:(NSObject *)aContainer
  ...
  if (self) {
    ...
    if (dirHead.totalBlocks % kBitsInOneBlock > 0)
      ++bitmapBlockCount;
    volumeBitmapBlocks =
        [[NSMutableArray alloc] initWithCapacity:bitmapBlockCount];
    NSUInteger i;
    for (i = 0; i < bitmapBlockCount; ++i)
      [volumeBitmapBlocks
          addObject:[blockStorage
                        mutableDataForBlock:dirHead.bitMapPointer + i]];
    ...
    directory.allEntriesVisible =
        [defaults boolForKey:ShowAllDirectoryEntriesKey];
  }
  return self;
}
  ...
  if ([entry isKindOfClass:[PDFileEntry class]]) {
    ...
    if (storageType == SEEDLING_FILE) {
      ...
      [self appendBlocksFromFileEntry:fileEntry
                       withIndexBlock:[blockStorage
                                          mutableDataForBlock:keyPointer]
                        isMasterIndex:NO
  ...
  for (i = 0; i < 256; ++i) {
    ...
    if (masterIndex) {
      if (blockNumber)
        [self appendBlocksFromFileEntry:fileEntry
                         withIndexBlock:[blockStorage
                                            mutableDataForBlock:blockNumber]
                          isMasterIndex:NO
                          toMutableData:allBlockData];
    } else {
      ...
      if (blockNumber == 0)
        ...
        if (skippedBlocks) {
          ...
          [allBlockData
              appendData:[NSMutableData dataWithLength:512 * skippedBlocks]];
          skippedBlocks = 0;
        }
        [allBlockData
            appendData:[blockStorage mutableDataForBlock:blockNumber]];
...
- (NSArray *)allocateBlocks:(NSUInteger)aBlockCount {
  ...
  NSLog(@"Allocated %lu blocks, Available blocks: %lu, Total blocks: %lu",
        (unsigned long)aBlockCount, (unsigned long)self.availableBlockCount,
        (unsigned long)self.totalBlockCount);
