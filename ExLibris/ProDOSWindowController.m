  ...
  if ([type isEqualToString:ProDOSFilePboardType]) {
    ...
    if (directory) {
      ...
      PDFileEntry *fileEntry =
          [[PDFileEntry alloc] initWithVolume:volume
...
- (IBAction)openGraphics:(id)sender {
  ...
  if (entry) {
    ...
    NSString *key =
        [NSString stringWithFormat:@"%@ Graphics", entry.description];
    ...
    if (!windowController) {
      ...
      windowController =
          [[GraphicsBrowseController alloc] initWithData:data
                                                    name:entry.fileName
                                               hasHeader:NO];
...
- (IBAction)getInfo:(id)sender {
  ...
  if (entry) {
    ...
    if (!windowController) {
      windowController =
          [[ProDOSInfoWindowController alloc] initWithEntry:entry];
      windowControllers[key] = windowController;
      [self.document addWindowController:windowController];
    }
    [windowController showWindow:self];
  ...
  if (entry) {
    ...
    if (!windowController) {
      ...
      if (data) {
        ...
        windowController =
            [[FileBrowseController alloc] initWithData:data
...
- (void)handleShowAllDirectoryEntriesChange:(NSNotification *)note {
  ...
  volume.directory.allEntriesVisible =
