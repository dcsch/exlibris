...
- (void)openForWriting {
  ...
  if (_fd <= 0 && _path != nil)
    _fd = open(_path.UTF8String, oflag,
...
- (NSMutableData *)mutableDataForBlock:(NSUInteger)blockIndex {
  ...
  if (_fd > 0) {
    ...
    if (_dosToProdosSectorMapping) {
      ...
      NSLog(@"Read block %lu (sector %d) (%ld bytes @ %lld)",
            (unsigned long)blockIndex, sector1, bytesRead, pos);
      ...
      NSLog(@"Read block %lu (sector %d) (%ld bytes @ %lld)",
            (unsigned long)blockIndex, sector2, bytesRead, pos);
    } else {
      off_t pos = _blockSize * blockIndex + _partitionOffset;
      lseek(_fd, pos, SEEK_SET);
      ssize_t bytesRead = read(_fd, buf, _blockSize);

      NSLog(@"Read block %lu (%ld bytes @ %lld)", (unsigned long)blockIndex,
            bytesRead, pos);
    }
...
- (BOOL)commitModifiedBlocks {
  ...
  for (NSNumber *blockNumber in _modifiedIndicies) {
    ...
    NSLog(@"Write block %lu (%ld bytes @ %lld)", (unsigned long)blockIndex,
          bytesWritten, pos);
  }
