...
- (void)windowDidLoad {
  ...
  } else if (typeId == BINARY_FILE_TYPE_ID || typeId == SYSTEM_FILE_TYPE_ID) {
...
- (void)hexDump {
  ...
  for (line = 0; line < lineCount; ++line) {
    ...
    [hexDumpString appendFormat:@"%02x %02x %02x %02x %02x %02x %02x %02x %02x "
...
- (void)catalog {
  ...
  NSString *dirName =
      [[NSString alloc] initWithBytes:ptr + 0x05
                               length:nameLen
  ...
  [catString appendString:@" NAME           TYPE  BLOCKS  MODIFIED         "
  ...
  do {
    for (; index < entriesPerBlock; ++index, ptr += entryLength) {
      ...
      dirName =
          [[NSString alloc] initWithBytes:ptr + 0x01
                                   length:nameLen
                                 encoding:[NSString defaultCStringEncoding]];
      BOOL locked = (ptr[0x1e] & 0x02) ? NO : YES;
      char *spc = "               ";
      [catString
          appendFormat:@"%c%@%s ", locked ? '*' : ' ', dirName, spc + nameLen];
      ...
      if (month > 0) {
        if (year >= 2000)
          ...
          year -= 1900;
        [catString appendFormat:@" %2ld-%s-%02ld %2ld:%02ld ", (long)day,
                                nameOfMonth + 4 * (month - 1), (long)year,
      ...
      // EOF
      [catString appendFormat:@"%8d ",
                              ptr[0x15] | (ptr[0x16] << 8) | (ptr[0x17] << 16)];
  ...
  [catString
      appendFormat:
          ...
          0, (unsigned long)volume.totalBlockCount];
